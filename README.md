...
- **两种阅读模式（同一套内容，两种讲法）**
  - **🙂 小白模式**：面向零基础大众。每页自动插入「30 秒看懂」卡（一句话看点 + 三步怎么做 + 常见坑 + 本页术语白话），正文专业术语带**悬停白话气泡**，字号与行距放宽、隐藏侧边目录；偏专业的页面会标注「已为你提炼要点」
  - **🎓 专业模式**（默认）：完整原理、公式、参数、评估标准与进阶细节
  - 切换方式：顶栏右侧 `🙂 小白模式 / 🎓 专业模式` 按钮（记忆偏好），或网址加 `?mode=simple` 直接进入
